...
| Explicit Gear stat modifiers | Audited Gear only | Gear | Partial foundation | `CombatStatsService` and the frontend stat helper can apply attached Gear Might/max-Health modifiers from explicit support metadata only. | No current starter Gear has an enabled stat modifier; Quick-Draw, Weaponmaster, replacement effects, and broad Gear text remain deferred. | `CombatStatsServiceTest`, `CombatResolverTest`, `cardDisplayStats.test.ts` | Raw rules text is not parsed into Gear bonuses. |

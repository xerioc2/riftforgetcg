...
- Tournament-style deck import/export and deck validation reports are available
  in the Deck Builder.
- Starter deck support audit tooling and `docs/CARD_RULES_BACKLOG.md` provide
  a code-backed implementation backlog for starter deck cards.
...
- Starter deck card audit: complete for current starter lists; implementation
  remains partial.
...
The current starter card inventory is generated by
`scripts/starter-support-audit.mjs` and checked into
`docs/CARD_RULES_BACKLOG.md`. Promote cards only after the relevant behavior is
scripted and covered by tests.

Phase A: basic units

- Start with `Vanguard Sergeant` and other no-text units as direct supported
  cases for play, movement, showdown participation, damage, and cleanup.
- Add direct tests before changing `CardSupportService` status.

Phase B: spells in starter decks

- Implement explicit spell scripts for stat modifiers, draw/card selection,
  bounce/return, and ready/exhaust effects.
- Keep counter/reaction-stack cards blocked until a stack/priority model exists.
- Add target/payment-specific tests for every promoted spell.

Phase C: gear/equipment

- Turn generic `[Equip]` recognition into an attachment/payment handler with
  legal target selection and attachment lifecycle tests.
- Keep non-equip or Hidden gear behavior partial until Hidden/reaction timing is
  modeled.

Phase D: Legend/Champion text

- Implement Legend and Champion activated/triggered hooks only after payment,
  ready/exhaust, Mighty, Deflect, and chosen-target timing are explicit.
- Keep identity/setup behavior separate from rules-text support.

Phase E: battlefields

- Add selected-battlefield effect hooks for conquer, hold, defend, and
  target-score modifiers.
- Gate battlefield status promotion on tests that verify timing and optional
  payment behavior.

Phase F: promote supported cards once scripted and tested

- Update `CardSupportService`, `docs/SUPPORTED_CARDS.md`, and the generated
  backlog notes together.
- Do not mark cards Supported based only on generic heuristics.
...
2. Starter deck Phase A basic-unit support and tests.
3. Starter deck spell buckets: stat modifiers, draw/card selection, bounce, and ready/exhaust.
4. Import/export polish for sideboards and alternate printing choices.

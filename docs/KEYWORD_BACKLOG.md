...
| EQUIP | Partial. Basic `[Equip]` gear is validated as a friendly battlefield Unit/Champion target, attaches through the generic play path, cannot fight as a unit, and attached gear moves to trash when its host is destroyed or returned to hand. Unsupported non-equip gear remains blocked. | Guardian Angel, Boots of Swiftness | Official attach timing, replacement/reattachment edge cases, payment-domain precision, detach edge cases, and gear card text. | High | P1 | Gear attaches only to legal friendly unit/champion; gear follows host death/bounce/trash; unattached gear cannot fight; unsupported gear stays blocked. |

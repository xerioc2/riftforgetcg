...
| DEATHKNELL | Partial overall. A basic death-event pipeline detects real deaths, skips bounce/return-to-hand, batches simultaneous combat deaths deterministically, and Loyal Poro's full printed draw trigger is card-specific Supported. Scuttle Crab logs as partial. | Scuttle Crab, Loyal Poro | Optional/mandatory choice handling, choosing opponent, reveal/facedown access, XP, and broader card-specific scripts. | High | P1 | Deathknell fires when a unit dies; does not fire on bounce; Loyal Poro draws exactly once when it did not die alone; Scuttle Crab remains partial. |

...
| HIDDEN | Partial. Cards with `[Hidden]` can be hidden from hand during Main by tapping a ready own rune. Hidden cards move to a dedicated hidden zone, are masked to opponents/spectators, are owner-previewable/inspectable, are not targetable, and do not fight or move through normal board actions. Public logs and copied debug info do not expose hidden identities. | Tideturner, Facebreaker | Official later play/react timing, reveal permissions, priority windows, hidden-card payment edge cases, and card-specific scripts. | High | P1 | Hidden card leaves hand, owner sees identity, opponent/spectator see only a masked hidden card/count, hidden card cannot be targeted/moved/fight, and later play remains blocked until timing exists. |
| DEATHKNELL | Partial overall. A keyword-driven death-event pipeline detects real Unit/Champion deaths, skips bounce/return-to-hand, batches simultaneous combat deaths deterministically, and dispatches card-specific effects through Deathknell handlers. Loyal Poro's full printed draw trigger and Lonely Poro's inverse "died alone" draw trigger are card-specific Supported; Scuttle Crab has a safe 1v1 alpha handler that privately reveals the opponent hand to the Crab controller. | Scuttle Crab, Loyal Poro, Lonely Poro | Multiplayer opponent choice, facedown access, XP, optional/mandatory choice handling, and broader card-specific scripts. | High | P1 | Deathknell fires when a unit dies; does not fire on bounce, reposition, or Gear cleanup; deployed Champions go to Trash when defeated instead of returning to Champion zone; Loyal Poro draws exactly once when it did not die alone; Lonely Poro draws exactly once when no other friendly Unit/Champion was at the death location; Scuttle Crab reveal is private and clears at end phase while XP/facedown remain deferred. |
...
| ACTION | Partial. Supported Action cards can be played by the active player in Main and by showdown participants during the simplified active-showdown action window. Stacked Deck is the first Action wired as a public chain opener; unsupported Action effects remain blocked. | Ride The Wind, Stacked Deck, Keeper's Verdict, Facebreaker | Formal priority, broad chain item creation, legal response windows, and card-specific Action prompts. | High | P0 | Supported Action spell legal on your turn and participant showdown window; Stacked Deck opens the narrow alpha chain; non-Action and unsupported Reaction cards rejected during showdown; unsupported Action effects stay blocked. |
...
| EQUIP | Partial. The alpha Equip lifecycle is implemented and tested: Gear is played to Base first, then attached with a separate Equip action from Base to a friendly Unit/Champion in Base or at the battlefield by paying its printed Equip cost. Attached Gear stays in Base with an attachment link, cannot fight or move directly, and returns to Base when its host leaves public play without triggering Gear Deathknell. Champion-zone identity cards are not legal equip targets. Unsupported non-equip gear remains blocked. | Guardian Angel, Boots of Swiftness | Official Equip timing, replacement/reattachment edge cases, Quick-Draw, Weaponmaster, voluntary detach rules, and complex gear card text. | High | P1 | Gear plays to Base, pays printed Equip costs, equips only from Base to legal friendly Unit/Champion, returns to Base on host death/bounce, unattached gear cannot fight, attached gear cannot be moved directly, unsupported gear stays blocked. |
...
| REACTION | Partial. Gust, Discipline, and En Garde are narrow chain-backed board-target Reactions, Defy is a narrow counter Reaction, and Not So Fast is a narrow targeted-spell counter Reaction: focused chain participants can play them onto the public chain opened by Stacked Deck when their exact target rules are met. Broad Reaction timing is still incomplete. | Gust, Discipline, En Garde, Defy, Not So Fast; other starter deck Reaction cards remain unsupported/partial | Official "any time" timing, broader counterspell targets, ability chain objects, Hidden/Ambush permissions, and additional card-specific Reaction scripts. | High | P0 | Gust returns a legal low-Might battlefield unit; Discipline gives +2 Might and draws 1; En Garde gives +1/+2 Might to a friendly unit; Defy can counter supported public spell chain items; Not So Fast can counter supported public enemy spell chain items that choose a friendly Unit/Champion Unit or Gear; all are illegal outside the current alpha chain window; ability-counter and broader counter behavior remain blocked. |

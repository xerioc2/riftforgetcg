...
| REACTION | Partial. Gust is the first chain-backed Reaction card: a focused chain participant can play it onto the public chain opened by Stacked Deck, and it returns a legal battlefield Unit/Champion with 3 Might or less when resolved. Broad Reaction timing is still incomplete. | Gust; other starter deck Reaction cards remain unsupported/partial | Official "any time" timing, counterspell targets, Hidden/Ambush permissions, broader spell/ability chain objects, and card-specific Reaction scripts. | High | P0 | Gust legal only during the current alpha chain focus window; illegal outside that window; target legality/fizzle behavior tested; counterspell cards remain blocked. |

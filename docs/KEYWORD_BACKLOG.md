...
| REACTION | Partial. Gust is the first chain-backed return Reaction, Defy is the first narrow counter Reaction, and Not So Fast is the first narrow targeted-spell counter Reaction: focused chain participants can play them onto the public chain opened by Stacked Deck when their exact target rules are met. Broad Reaction timing is still incomplete. | Gust, Defy, Not So Fast; other starter deck Reaction cards remain unsupported/partial | Official "any time" timing, broader counterspell targets, ability chain objects, Hidden/Ambush permissions, and additional card-specific Reaction scripts. | High | P0 | Gust legal only during the current alpha chain focus window; Defy can counter supported public spell chain items; Not So Fast can counter supported public enemy spell chain items that choose a friendly Unit/Champion Unit or Gear; all are illegal outside that window; ability-counter and broader counter behavior remain blocked. |

...
| REACTION | Partial. Gust is the first chain-backed return Reaction and Defy is the first narrow counter Reaction: focused chain participants can play them onto the public chain opened by Stacked Deck. Broad Reaction timing is still incomplete. | Gust, Defy; other starter deck Reaction cards remain unsupported/partial | Official "any time" timing, broader counterspell targets, Hidden/Ambush permissions, spell/ability chain objects, and card-specific Reaction scripts. | High | P0 | Gust legal only during the current alpha chain focus window; Defy can counter supported public spell chain items; both are illegal outside that window; Not So Fast and broader counter behavior remain blocked. |

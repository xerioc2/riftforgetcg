...
| EQUIP | Partial. Basic `[Equip]` gear is not blocked and can attach through generic support paths. | Guardian Angel, Boots of Swiftness | Official attach timing, payment, target selection, attachment cleanup, and gear text. | High | P1 | Gear attaches only to legal friendly unit; gear goes to trash with host if required; unattached gear cannot fight. |

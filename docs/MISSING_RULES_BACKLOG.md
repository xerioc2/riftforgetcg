...
| Item | Status | Why it matters | Likely files | Suggested first test |
| --- | --- | --- | --- | --- |
| Multi-location Battlefield model | Partial foundation / Post-alpha | Current alpha renders three shared Battlefield lanes and sends drag-to-lane move destinations. Server-side `battlefieldLocationId` scopes movement, showdowns, combat resolution, controller keys, and scoring, but official-style 1v1 support still needs selected Battlefield instances per objective, hidden slots, bot/UI strategy, Battlefield effects, and full "here" text. This is not the same as 3+ player multiplayer. | `LiveGameState`, `CardInstance`, `GameEngine`, `CombatResolver`, `RulesValidator`, `BotService`, `GameBoard.tsx` | Moving into Battlefield A starts a showdown only with opposing units at Battlefield A, not Battlefield B. |

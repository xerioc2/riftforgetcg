...
| Star-Crossed | 2 | Playable for alpha testing, but card-specific behavior may be incomplete. | Staged friendly/enemy return exists in current alpha card scripts. | Full official Reaction timing and edge cases. | Medium | OK with caveat |
...
| Eclipse | 2 | Playable for alpha testing, but card-specific behavior may be incomplete. | Card is playable by alpha support gate. | Exact spell effect needs audit. | Medium | OK with caveat |

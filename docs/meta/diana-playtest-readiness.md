...
| Last Rites | 1 | Partial: alpha Equip lifecycle supports the exact printed Chaos rune plus recycle-2-from-Trash Equip cost. | Plays to Base, then equips from Base by paying a Chaos rune and recycling exactly two owned Trash cards to the bottom of the Main Deck pool. | Full official Equip timing and any additional card-specific edge cases remain deferred. | Medium | OK with caveat |

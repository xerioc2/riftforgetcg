...
const PARTIAL_REASONS = new Map([
  ...
  ['LAST RITES', 'Partial: alpha Equip lifecycle supports the exact printed Chaos rune plus recycle-2-from-Trash equip cost. Recycled cards move to the bottom of the owner\'s Main Deck pool, and full official Equip timing or additional card-specific effects remain deferred.'],
  ['ABANDONED HALL', 'Partial: exact spell-play optional trigger is implemented for active Battlefield lanes. The spell\'s controller may choose a friendly public Unit/Champion here to get +1 Might this turn; full official trigger stacking and broader Battlefield rules remain deferred.'],
  ...
  ['STAR-CROSSED', 'Partial: alpha chain-window Reaction support exists for returning one friendly public battlefield Unit/Champion and one enemy public battlefield Unit/Champion to their owners\' hands. Full official any-time Reaction timing remains incomplete.'],
  ['ECLIPSE', 'Partial: alpha chain-window Reaction support exists for giving one public battlefield Unit/Champion -4 Might this turn, then creating a private owner-only Predict choice. Full official any-time Reaction timing remains incomplete.'],
  ['STUPEFY', 'Partial: alpha chain-window Reaction support exists for giving one public battlefield Unit/Champion -1 Might this turn, to a minimum of 1, and drawing 1 privately. Full official any-time Reaction timing remains incomplete.'],
  ['MOONFALL', 'Partial: exact Action support chooses an active battlefield where you have a public Unit/Champion, may move one enemy public battlefield Unit/Champion there, then gives enemy units there -2 Might this turn. Full official timing and broader movement edge cases remain incomplete.'],
...
function isUnsupportedAction(card) {
  ...
  const supportedEffect = text.includes(':rb_might:')
    ...
    || isFlash(card)
    || isStackedDeck(card);
  return (text.includes('counter a spell') && !isDefy(card) && !isAbandon(card) && !isHardBargain(card))
...
function isAbandon(card) {
  const text = String(card?.rulesText ?? '').toLowerCase();
  return normalizeName(card?.name) === 'ABANDON'
...
function isHardBargain(card) {
  const text = String(card?.rulesText ?? '').toLowerCase();
  return normalizeName(card?.name) === 'HARD BARGAIN'
...
function isDefiantDance(card) {
  const text = String(card?.rulesText ?? '').toLowerCase();
  return normalizeName(card?.name) === 'DEFIANT DANCE'
...
function isFlash(card) {
  const text = String(card?.rulesText ?? '').toLowerCase();
  return normalizeName(card?.name) === 'FLASH'
    && text.includes('move up to 2 friendly units')
    && text.includes('base');
}
...
function isZhonyasHourglass(card) {
  const text = String(card?.rulesText ?? '').trim().toLowerCase();
  return normalizeName(card?.name) === "ZHONYA'S HOURGLASS"

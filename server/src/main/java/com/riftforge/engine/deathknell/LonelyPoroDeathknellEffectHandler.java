...
@Component
public class LonelyPoroDeathknellEffectHandler implements DeathknellEffectHandler {
  @Override
  public boolean supports(CardDefinition card) {
    return card != null && card.name() != null && card.name().trim().equalsIgnoreCase("Lonely Poro");
  }
  ...
  public void resolve(LiveGameState state, DeathEvent death, DeathknellEffectContext context) {
    if (death.hadOtherFriendlyUnitAtLocation()) {
      context.log(state, death.ownerId(), "Lonely Poro did not die alone. Deathknell did not draw.");
      return;
    }
    context.autoDraw(state, death.ownerId(), playerName(state, death.ownerId()) + "'s deck is empty - no Deathknell draw.");
    context.log(state, death.ownerId(), "Lonely Poro's Deathknell drew 1.");
  }

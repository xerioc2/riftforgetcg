...
export function PhaseBar({ currentPhase, isMyTurn, opponentName, onPassPhase }: PhaseBarProps) {
  return (
    <div className="pointer-events-auto absolute bottom-[172px] left-0 z-20 flex h-12 items-center gap-1 border-t border-line bg-panel/95 px-3" style={{ right: '280px' }}>
      ...
      <div className="ml-auto flex items-center gap-3">
        <span className="hidden whitespace-nowrap text-xs text-slate-400 lg:inline">{isMyTurn ? 'Your turn' : `${opponentName}'s turn`}</span>
        <button className="btn-primary min-h-7 px-4 py-1 text-xs disabled:opacity-40" disabled={!isMyTurn} onClick={onPassPhase}>
          Pass
        </button>
      </div>
    </div>
